...
import type { GoalHooks } from "./goal.hooks"
...
/**
 * Set hooks for Goal routes
 */
export function setGoalHooks(h: GoalHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "name", "type", "targetAmount", "currentAmount", "deadline", "priority", "isCompleted", "createdAt", "updatedAt"] as const
...
/**
 * Create Goal CRUD routes
 */
export function createGoalRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Goal not found" } }, 404)
      }
