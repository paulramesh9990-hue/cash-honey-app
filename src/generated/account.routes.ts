...
import type { AccountHooks } from "./account.hooks"
...
/**
 * Set hooks for Account routes
 */
export function setAccountHooks(h: AccountHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "name", "type", "balance", "currency", "isActive", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["user", "transactions", "portfolios"] as const
...
export function createAccountRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Account not found" } }, 404)
      }
